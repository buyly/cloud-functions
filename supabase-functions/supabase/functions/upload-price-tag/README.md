# Upload Price Tag Function

This Supabase Edge Function generates presigned URLs for uploading price tag images to Cloudflare R2.
...
```bash
cd supabase/functions/upload-price-tag
deno cache index.ts
```
...
```bash
supabase functions deploy upload-price-tag
```
...
curl 'https://zbisfbcmgypcxokydnwc.supabase.co/functions/v1/upload-price-tag' \
...
```bash
supabase start
supabase functions serve upload-price-tag --env-file .env
```
