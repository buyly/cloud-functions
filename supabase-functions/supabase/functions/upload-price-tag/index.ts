...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Generate unique filename for price tag image
    const filename = `price-tag-${Date.now()}-${crypto.randomUUID()}.${extension}`;

    console.log("Generating presigned URL for price tag:", filename);
    ...
    console.log("Presigned URL for price tag generated successfully");
    ...
    console.error("Error generating upload URL for price tag:", error);
    return new Response(
      JSON.stringify({
